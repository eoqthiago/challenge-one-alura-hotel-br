...
public class Buscar extends JFrame {
				...
				switch(validaBusca(entrada)) {
					...
					case 1:
						...
						if (panel.getSelectedIndex() == 0) {
							buscarId(Long.parseLong(entrada));
						} else {
					  	break;
						}
				}
	...
	private void preencherTabelaHospedes() {
		...
		try {
			for (Hospede hospede : hospedes) {
				modeloHospedes.addRow(new Object[] { hospede.getId(), hospede.getNome(), hospede.getSobrenome(), hospede.getDataNascimento(), hospede.getNacionalidade(), hospede.getTelefone(), hospede.getReserva() });
			}
		} catch (Exception e) {
			throw e;
		}
	...
	public void buscarId(Long id) {
	    try {
	        ...
	        if (reserva != null) {
	            ...
	            // Trate o caso em que a reserva não é encontrada
	            // Por exemplo, exiba uma mensagem de erro ou faça alguma outra ação apropriada
	        }
	    } catch (Exception e) {
	        e.printStackTrace();
	        // Trate a exceção de acordo com as necessidades do seu programa
	    }
	}
	...
	public List<Reserva> listarReservas() {
		return this.reservaController.buscarTodos();
	}
	...
	private void deletarHospede() {
		...
		if (objetoDaLinhaHospedes instanceof Integer) {
			Long id = (Long) objetoDaLinhaHospedes;
